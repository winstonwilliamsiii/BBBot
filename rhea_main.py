...
def run_analysis(symbol: str, headlines: str, log_mlflow: bool = True) -> str:
    """Analyze a ticker with Rhea's full model stack and return JSON."""
    RheaBot, RheaConfig = _load_rhea_bot()
    config = RheaConfig.from_env()
    bot = RheaBot(config)
    parsed_headlines = [
        line.strip() for line in str(headlines).splitlines() if line.strip()
    ]
...
def _gradio_analyze(symbol: str, headlines: str, log_mlflow: bool) -> str:
    if not symbol.strip():
...
def launch_gradio(share: bool = False) -> None:
    ...
    with gr.Blocks(title="Rhea Bot — MLflow + Gradio") as demo:
        ...
        with gr.Tab("Ticker Analysis"):
            ...
            analysis_out = gr.Code(language="json", label="Analysis Result")
