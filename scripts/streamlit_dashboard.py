...
# RBAC imports
try:
    from frontend.utils.rbac import show_login_form, show_user_info
...
with col1:
    ...
    st.markdown(
        "Monitor your personal finances with detailed budget analysis."
    )
...
tab1, tab2, tab3 = st.tabs(
    ["💸 Transactions", "📊 Watchlist", "🤖 Bot Status"]
)
            ...
            if st.button("➕ Add Transaction"):
                if user_id and amount:
                    result = create_transaction(
                        user_id, amount, str(date_input)
                    )
                    if result.get("success"):
                        st.success("✅ Transaction added!")
                    else:
                        st.error(
                            result.get("error", "Failed to add transaction")
                        )
                else:
                    st.warning("User ID and amount required")
    else:
        st.info(
            "💡 Appwrite services not configured. "
            "Add your Function IDs to `.env` to enable transactions."
        )
...
with tab2:
    if APPWRITE_SERVICES_AVAILABLE:
        ...
        with col_d:
            if st.button("📋 View Watchlist"):
                if user_id_wl:
                    ...
                    if wl.get("success"):
                        st.success(
                            f"Found {len(wl.get('watchlist', []))} items"
                        )
        ...
        st.info(
            "💡 Appwrite services not configured. "
            "Add your Function IDs to `.env` to enable watchlist."
        )

with tab3:
    st.subheader("Trading Bot Status")
    create_custom_card(
        "🤖 Bot Status", "Visit the Trading Bot page for full controls"
    )
    st.info(
        "Navigate to **🤖 Trading Bot** page in the sidebar "
        "for detailed bot management."
    )
