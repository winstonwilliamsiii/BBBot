...
def read_mysql_data():
    ...
    df = pd.read_sql("SELECT * FROM binance_ohlcv ORDER BY timestamp DESC LIMIT 100", conn)
...
# === Airflow Tasks ===
t1 = PythonOperator(task_id="trigger_airbyte", python_callable=trigger_airbyte_sync, dag=dag)
t2 = PythonOperator(task_id="read_mysql", python_callable=read_mysql_data, dag=dag)
t3 = PythonOperator(task_id="compute_indicators", python_callable=compute_indicators, dag=dag)
t4 = PythonOperator(task_id="execute_trade", python_callable=execute_trade, dag=dag)
t5 = PythonOperator(task_id="log_mlflow", python_callable=log_to_mlflow, dag=dag)
