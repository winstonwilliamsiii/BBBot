...
router = APIRouter(tags=["mansa-router"])
...
def _submit_alpaca_order(payload: BrokerOrderRequest) -> dict[str, Any]:
    if payload.dry_run:
        return _normalized_execution_response(
            ...
            mode=(
                "paper"
                if _as_bool(os.getenv("ALPACA_PAPER"), True)
                else "live"
            ),
        )
    ...
    if not api_key or not secret_key:
        raise HTTPException(
            status_code=503,
            detail=(
                "Missing Alpaca credentials: ALPACA_API_KEY / "
                "ALPACA_SECRET_KEY"
            ),
        )
    ...
    if not result:
        raise HTTPException(
            ...
            detail=(
                "Alpaca order failed: "
                f"{connector.last_error or 'unknown error'}"
            ),
        )
...
def _resolve_ibkr_conid(client: Any, payload: BrokerOrderRequest) -> int:
    ...
    exchange = payload.exchange or str(
        payload.metadata.get("exchange") or "IDEALPRO"
    )
    if len(cleaned_symbol) == 6:
        resolved = client.resolve_forex_conid(
            payload.symbol,
            exchange=exchange,
        )
        if resolved:
    ...
    raise HTTPException(
        status_code=400,
        detail=(
            "IBKR orders require `conid` for non-FOREX symbols. "
            "Provide `conid` directly or use a 6-character forex "
            "symbol for auto-resolution."
        ),
    )
...
def _submit_ibkr_order(payload: BrokerOrderRequest) -> dict[str, Any]:
    gateway_url = (
        _clean_env_text(os.getenv("IBKR_GATEWAY_URL"))
        or "https://localhost:5000"
    )
    account_id = payload.account_id or _clean_env_text(
    ...
    if not result:
        raise HTTPException(
            status_code=502,
            detail=(
                "IBKR order failed or gateway authentication "
                "is unavailable"
            ),
        )
...
@router.post("/ml/randomforest/predict")
